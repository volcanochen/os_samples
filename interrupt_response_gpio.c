...
static enum hrtimer_restart hrtimer_callback(struct hrtimer *timer) {
    // 触发GPIO引脚电平变化以触发硬件中断
    gpio_set_value(GPIO_PIN_NUMBER, !gpio_get_value(GPIO_PIN_NUMBER));
...
static int __init interrupt_response_time_init(void) {
    ...
    // 初始化GPIO引脚
    ret = gpio_request(GPIO_PIN_NUMBER, "gpio_interrupt");
    if (ret) {
    ...
    ret = gpio_direction_output(GPIO_PIN_NUMBER, 0);
    if (ret) {
